...
usersController.getAll = async (req, res) => {
  ...
  try {
    ...
    const redisResult = await redis.hGetAll("users");
...
usersController.getSingleUser = async (req, res) => {
  ...
  try {
    ...
    const redisResult = await redis.hGet("users", name);
    res.status(200).send({
      code: 200,
      message: "Successful",
      data: user, redisResult
    });
  } catch (error) {
    console.log("error", error);
    return res.status(500).send(error);
  ...
  try {
    ...
    if (!result) {
      ...
      if (bcrypt.compareSync(body.password, result.password)) {
        ...
        await redis.hSet('users' , username, JSON.stringify(result));
